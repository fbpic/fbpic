...
from fbpic.lpa_utils.laser import add_laser_pulse, GaussianLaser, \
            LaguerreGaussLaser, DonutLikeLaguerreGaussLaser
...
def propagate_pulse( Nz, Nr, Nm, zmin, zmax, Lr, L_prop, zf, dt,
        N_diag, w0, ctau, k0, E0, m, N_show, n_order, rtol, boundaries,
        v_window=0, use_galilean=False, v_comoving=None, show=False ):
    """
...
def fit_fields( sim, m ) :
    ...
    laser_profile = np.sqrt( dz*(abs( grid.Er )**2).sum(axis=0) )
...
def show_fields( sim, fieldtype, m ):
    """
    Show the field `fieldtype` on the interpolation grid

    Parameters
    ----------
    grid: an instance of a Simulation object
