...
\comment{See if plares also uses that}
...
(\cref{sec:vacuum_vg}). The plasma density was $n_e = 1.75\times
...
velocity velocity exhibits an very weak dependence on resolution
...
small here and may thus seem unimportant, this difference determines
the dephasing length in a laser-wakefield simulation, and it is thus
...
codes, and is often dealt with by either using an even finer grid in $z$ or
a \emph{coarser} grid in $r$. \comment{Say no need to compromise in
  this code.}
