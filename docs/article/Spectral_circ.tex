...
the core principle of Cartesian spectral algorithms, including the PSATD algorithms.
...
\hat{F}_{\theta,m}(r,z) & = i\;\mathrm{IFT} \left[ \; \mathrm{IHT}_{m+1}
                         [ \spectral{F}_{+,m}(k_\perp,k_z) ] - \mathrm{IHT}_{m-1} [ \spectral{F}_{-,m}(k_\perp,k_z) ] \; \right]
\end{align*}
...
Transform (FFT) algorithm. The implementation we chose for the
Discrete Hankel Transform (DHT) is described in the next section
...
\subsection{Trasnverse discretization of the intermediate grid, of the spectral grid
  and of the Hankel Transform}
\label{sec:discretization}

FFT imposes evenly spaced grid both in z and kz. On the other hand,
...
\end{itemize}
Here, we choose to discretize the algorithm on a even grid in
space
...
but on a non-even grid in $k$ space. This is because the authorized
...
evenly-space $k$. Here, for simplicity, we impose that the
...
\caption{\label{fig:Kgrid}Position of the grid points in $k$ space
  ...
  evenly-spaced grid used in Cartesian spectral codes (grey dots, $k_j = j\pi/r_{max}$)}
\end{figure}
...
\frac{k_\perp}{2} \spectral{F}^{n+1/2}_m
\qquad
\spectral{j}'^{n+1/2}_{-,m} = \spectral{j}^{n+1/2}_{-,m} - \frac{k_\perp}{2} \spectral{F}^{n+1/2}_m
\qquad \spectral{j}'^{n+1/2}_{z,m} = \spectral{j}^{n+1/2}_{z,m} - ik_z
\spectral{F}^{n+1/2}_m\]
with
\[ \spectral{F}^{n+1/2}_m = - \frac{1}{k_\perp^2 + k_z^2}\left(
