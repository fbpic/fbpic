...
\item a second, separate modification was introduced by the development
  ...
  Maxwell equations in Fourier space. \comment{(These algorithm are also sometimes referred to as
...
\end{equation}
\end{subequations}
\noindent \comment{For particles that are in the lower half of the first
...
(e.g. quadratic or cubic) could also be used in principle, \comment{with
...
\comment{which would be an adaptation of the Cartesian PSTD scheme \cite{Liu}.}
\begin{subequations}
\begin{align}
\tE{+}{n+1} = \; & \tE{+}{n} + 
...
% add references (web sites) for Python, FFTW, BLAS and Numba
...
$\vec{k}$ space \citep{LehePRSTAB2013,GodfreyJCP2013,XuCPC2013} on the lower left
...
% show also spectrum of PSATD-circ run?
%\end{figure}
