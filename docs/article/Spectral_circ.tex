...
which is typically used in a spectral Cartesian code. In practice, for $N_x = N_y =
...
it in a number of physical situations. In particular, we focused on
tests of the dispersion relation, since the absence of
spurious numerical dispersion was one of the original goals of this algorithm.

\subsection{Propagation in vacuum}

A first test consisted in letting a laser pulse propagate and in
measuring its group velocity in the simulation. The simulations were
run with the spectral cylindrical algorithm presented in
\cref{sec:implementation}. For comparison, the same simulations 
were also run with a finite-difference cylindrical
...
The simulations were run with a moving window, in a box 
The resolution was varied while keeping the same cell aspect ratio ($\Delta r = 5\Delta z$). The
timestep is set to $\Delta t = \Delta z/c$ in the case of the spectral
cylindrical algorithm and to $\Delta t = 1/\sqrt{}$ in the case of the finite-difference
algorithm (due to the existence of a Courant
limit for this algorithm). 

was The laser properties were

Figure : represent box and group velocity.

Due to the finite waist, the group velocity of the laser on axis. 

\subsection{Propagation in a plasma}

Mention moving window. (In the case of our
spectral cylindrical algorithm, the fields were progressively set to zero at the
left boundary of the moving window, in order to prevent them from
wrapping around at the right boundary, as a consequence of the
