...
%\pagestyle{empty}
\author[1]{R\'emi \textsc{Lehe}}
\author[2]{Manuel \textsc{Kirchen}}
\author[3]{Igor A. \textsc{Andriyash}}
\author[4]{Brendan B. \textsc{Godfrey}}
\author[1]{Jean-Luc \textsc{Vay}}

\affil[1]{Lawrence Berkeley National Laboratory, Berkeley, California 94720, USA}
\affil[2]{DESY, 22607 Hamburg, Germany}
\affil[3]{LOA, 91761 Palaiseau, France}
\affil[3]{University of Maryland, College Park, MD 20742, USA}
...
time, and thus the force on a co-propagating electron bunch should be
correct.

In order to confirm this prediction, we ran a test simulation in which a single
...
pulse is characterized by a waist $w_0 = 25\;\mathrm{\mu m}$,
...
$\gamma_e = 25$. The simulation was run, again, with both our spectral
...
longitudinal and transverse size of 40 and 60 $\mathrm{\mu m}$ respectively.
...
\caption{\label{fig:Laser}Evolution of the transverse normalized
  ...
  laser pulse (see \cref{fig:Schematic_laser} for a
  ...
  different resolutions in the simulation.}
\end{figure}
...
We acknowledge Axel Huebl for insightful discussions on the GPU implementation.
