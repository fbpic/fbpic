...
and is instead often dealt with by using
  3D Cartesian PIC algorithms. However, 3D PIC algorithms are very 
...
Importantly, while finite-difference algorithm often suffer from
...
%However, due to the
%lower number of cells in a 2D array, as compared to a 3D array, one can drastically reduce the
...
timesteps. The subsections 
\cref{sec:sec:gathering,sec:eq-motion,sec:current-deposition,sec:FieldIntegration} 
...
velocity is obtained, without having to specifically adapt the
resolution in $r$.
...
suppressed \citep{XuJCP2013,YuJCP2014,YuCPC2015,
GodfreyIEEE2014,GodfreyJCP2014}. These high-order effects can lead to
...
simulations. It is in fact likely that these instabilities may not have enough time to
...
high-frequency radiation. This radiation is due to the
zero-order numerical Cherenkov effect, a fact which is confirmed by
the observation of the corresponding characteristic parabola in
$\vec{k}$ space \citep{LehePRSTAB2013,XuJCP2013}, on \cref{fig:Cherenkov-spectrum}. 
Importantly, the amplitude of this unphysical field
...
This confirms the fact that our spectral
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.9\textwidth]{figures/Cherenkov-spectrum.png}
\caption{\label{fig:Cherenkov-spectrum}Spectrum of the electric field in the
  finite-difference simulation. This spectrum corresponds to the
  fields plotted in the left panels of \cref{fig:Cherenkov}. We used
  \cref{eq:CircFwTransp} to calculate the spectral component $\mathcal{E}_{+,1}$
  from the electric field $E$ in real-space. The spectrum displays the
characteristic parabola of the zero-order Cherenkov effect.}
\end{figure}
