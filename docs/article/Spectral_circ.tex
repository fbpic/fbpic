...
\comment{Not the same parameters : waist is 20 microns}

In order to further ascertain that the spectral algorithm described here gives
appropriate results, we compared the laser-wakefield in simulations
of the previous subsection (\cref{sec:linear_plasma}) with the corresponding analytical predictions. 

Since here $a_0 = 10^{-2}$, the analytical wakefield is given by
the linear and quasistatic theory. For a laser of the
form $a(z, r, t)= a_\ell(z, t) e^{-r^2/w_0^2} \cos(k_0z-\omega_0 t)$,
where $a_\ell(z, t)$ is the longitudinal envelope of the laser, this yields \citep{EsareyRMP2009}
\begin{equation} 
...
Here, the longitudinal profile $a_\ell$ was extracted directly from the
...
on \cref{fig:Linear_wkfld}. These predicted curves are compared with
the $E_z$ and $E_y$ field from the simulation (red lines).
...
\caption{\label{fig:Cherenkov} \comment{Add description}.}
\end{figure}
