...
\author[**]{Manuel \textsc{Kirchen}}
\author[***]{Igor A. \textsc{Andriyash}}
\author[*]{Jean-Luc \textsc{Vay}}

\affil[*]{Lawrence Berkeley National Laboratory, Berkeley, California 94720, USA}
\affil[**]{DESY, 22607 Hamburg, Germany}
\affil[***]{Laboratoire d'Optique Appliqu\'ee, 91761 Palaiseau, France}

\begin{document}

\maketitle

\comment{Read over again}
\comment{Add abstract}
...
charge was found to be almost the same (\comment{XXX pC} with the
finite-difference algorithm and \comment{XXX pC} with the spectral
...
\caption{\label{fig:Cherenkov}Snapshot of a finite-difference
  ...
  bunch in more details. \comment{Update figures + Put more
    transparency on the bunch in the lower plots ?}}
\end{figure}
...
\subsection{Accurate force of a laser on a co-propagating electron}
...
overlap with it \citep{Cipiccia,Nemeth}. It is also the typical
...
However, in the appendix of \citep{LehePRSTAB2014}, it was shown that
finite-difference PIC codes tend 

Spurious force. To evaluate it, we look at the conservation (say order
...
\caption{\label{fig:Schematic_laser}Representation of the situation
  which is simulated in \cref{sec:accurate-laser}. The laser
...
\caption{\label{fig:Laser}Evolution of the transverse normalized
  momentum of a macroparticle $p_x/m_e c$, as it copropagates with a
  ...
  algorithm respectively, while the different curves correspond to
  different resolutions. \comment{Update figure}}
\end{figure}
...
We thank Brendan Godfrey for very valuable comments and suggestions. We
also acknowledge Axel Huebl for insightful discussions on the GPU implementation.
