...
axis and $\mathrm{HT}_{m}$ represents a Hankel Transform of order
$m$ along the $r$ axis, and where $\mathrm{IFT}$ and
$\mathrm{IHT}_{m}$ represent the corresponding inverse
transformations:
...
transform (e.g. \cite{Cree,Siegman,Guizar}), and the use of one or the other
...
space
\[ r_j = r_{max} \frac{j+1/2}{N_r} \qquad  j \in \{0, ..., N_r-1 \} \]
...
regularly-space $k$.) If, for simplicity, we impose a perfectly conducting boundary at $r_{max}$
(i.e. $E_z(r_{max}, z) = 0$), it turns out that the set of authorized
$k_\perp$ depend on the mode $m$ and reads
\[  k^m_{\perp,j} = \frac{\alpha_j^m}{r_{max}} \qquad j \in \{0, ..., N_r-1 \}\]
where $\alpha^m_j$ is the $j$th strictly positive zero of the Bessel function of order
$m$ ($J_m$). These values are represented \textbf{Do a plot}. Notice
that the fact that the $k$s depend on the mode number $m$ is not a problem since each mode evolves separately in \cref{eq:CircMaxwellp,eq:CircMaxwellm,eq:CircMaxwellz}.
...
\[ \mathrm{DHT^m_n}[f] \,(k^m_j) = \sum_{\ell=0}^{N_r-1} M_{j,\ell}(n,m)
\,f(r_\ell) \qquad \mathrm{IDHT^m_n}[g] \, (r_j) = \sum_{\ell=0}^{N_r-1}
M^{-1}_{j,\ell}(n,m) \,g(k^m_\ell) \]
\textbf{Explain that $n$ is different than $m$.}

Notice that the transformation matrix $M(n,m)$ depends on $n$ (order of the Hankel transform) and $m$ (index
of the mode that is being transform). The expression of these matrices,
...
the boundary conditions. In practice, $M(n,m)$ and $M^{-1}(n,m)$ need to be
...
matrix multiplication is proportional to $N_r^2$, which is slow compared to an FFT ($N_r \log(N_r)$), but
