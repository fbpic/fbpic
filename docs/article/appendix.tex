...
Hankel Transform (IDHT) retrieve the initial function. As explained in
...
is valid for any value of $n$, $m$, $j$, $\ell$, except in the case
...
\paragraph{In the case where $n=0$ or $m=0$:}
In this case, \cref{eq:DHT-matrix} is applicable for any $j$ and
$\ell$, and we impose it for 

imposes $N_r^2$ conditions on the matrix
