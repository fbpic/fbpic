...
def add_linear_envelope_gather_for_mode( m, F,
    ...
    F: float
        The fields felt by one macroparticle, which represent a or grad_a
        (before the contribution of mode `m` has been added)
    ...
    F_grid: 2darrays of complexs
        The field on the interpolation grid for mode `m`
    ...
    --------
    F: float
        The field felt by one macroparticle, which represent a and grad_a
        (after the contribution of mode `m` has been added)
    """
    # Create temporary variables
    # for the "per mode" gathering
