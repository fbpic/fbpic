...
def deposit_J_gpu_unsorted(x, y, z, w, q,
    ...
    if i < w.shape[0]:
        ...
        J_t_m_00 = Sr_linear(r_cell, 0, -f, bn) * \
            Sz_linear(z_cell, 0) * J_t_m_scal
        J_r_m_00 = Sr_linear(r_cell, 0, -f, bn) * \
            Sz_linear(z_cell, 0) * J_r_m_scal
        J_z_m_00 = Sr_linear(r_cell, 0, f, bn) * \
            Sz_linear(z_cell, 0) * J_z_m_scal
        J_r_m_01 = Sr_linear(r_cell, 0, -f, bn) * \
            Sz_linear(z_cell, 1) * J_r_m_scal
        J_t_m_01 = Sr_linear(r_cell, 0, -f, bn) * \
            Sz_linear(z_cell, 1) * J_t_m_scal
        J_z_m_01 = Sr_linear(r_cell, 0, f, bn) * \
            Sz_linear(z_cell, 1) * J_z_m_scal

        J_r_m_10 = Sr_linear(r_cell, 1, -f, bn) * \
            Sz_linear(z_cell, 0) * J_r_m_scal
        J_t_m_10 = Sr_linear(r_cell, 1, -f, bn) * \
            Sz_linear(z_cell, 0) * J_t_m_scal
        J_z_m_10 = Sr_linear(r_cell, 1, f, bn) * \
            Sz_linear(z_cell, 0) * J_z_m_scal
        J_r_m_11 = Sr_linear(r_cell, 1, -f, bn) * \
            Sz_linear(z_cell, 1) * J_r_m_scal
        J_t_m_11 = Sr_linear(r_cell, 1, -f, bn) * \
            Sz_linear(z_cell, 1) * J_t_m_scal
        J_z_m_11 = Sr_linear(r_cell, 1, f, bn) * \
            Sz_linear(z_cell, 1) * J_z_m_scal
...
# --------------------------------------------------
# Field deposition - linear - J from particle momenta
# --------------------------------------------------


@compile_cupy
def deposit_J_gpu_unsorted_momentum(x, y, z, w, q,
                                    ux, uy, uz, inv_gamma,
                                    invdz, zmin, Nz,
                                    invdr, rmin, Nr,
                                    j_r_m, j_t_m, j_z_m, m,
    ...
    ux, uy, uz : 1darray of floats (dimensionless)
        The normalized momentum of the particles

    inv_gamma : 1darray of floats (dimensionless)
        The inverse Lorentz factor of the particles
    ...
    if i < w.shape[0]:
        ...
        vxj = c * ux[i] * inv_gammaj
        vyj = c * uy[i] * inv_gammaj
        vzj = c * uz[i] * inv_gammaj
        # Weights
        wj = q * w[i]
