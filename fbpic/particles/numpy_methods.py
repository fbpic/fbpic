...
def push_p_numpy( ux, uy, uz, inv_gamma,
    ...
    # Get the new u
    ux = s*( ux + taux*utau + uy*tauz - uz*tauy )
    uy = s*( uy + tauy*utau + uz*taux - ux*tauz )
    uz = s*( uz + tauz*utau + ux*tauy - uy*taux )
...
def push_x_numpy( x, y, z, ux, uy, uz, inv_gamma, dt ) :
    ...
    # Particle push
    x = x + chdt*inv_gamma*ux
    y = y + chdt*inv_gamma*uy
    z = z + chdt*inv_gamma*uz
